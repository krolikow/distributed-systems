...
public class EventSubscriptionServer
{
	private static final Logger logger = Logger.getLogger(EventSubscriptionServer.class.getName());
	...
	private void start() throws IOException 
	{
		...
		try{
			EventSubscriberImpl eventSubscriber = new EventSubscriberImpl();
			server = NettyServerBuilder.forAddress(socket).executor(Executors.newFixedThreadPool(16))
					.addService(eventSubscriber)
					.build()
					.start();
		}
		catch (Exception e){
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			...
			public void run() {
				...
				EventSubscriptionServer.this.stop();
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		final EventSubscriptionServer server = new EventSubscriptionServer();
		server.start();
